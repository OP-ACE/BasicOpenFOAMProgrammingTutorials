...
int main(int argc, char *argv[])
{
	...
	volScalarField myp // note that pressure is a scalar field
	(
		IOobject
		(
		    "p", // name of the field
		...
		mesh //! 场的初始化:
		     //! 初始化(值为0;数组大小和网格大小相匹配)
		     //! initialises the field to match the size of the mesh with default (0) values
	);

    
    
    Info<< "Reading field U\n" << endl;
	volVectorField myU // note that velocity is a vector field
	(
		IOobject
		(
		    "U",
	...
	// compute distance of each cell centre from x0; 
	// units of mesh.C() are those of length, 
	// as this field describes position in the Cartesian reference frame.

	// 距原点最远单元的距离
	const scalar rFarCell = max( 
		mag(dimensionedVector("x0",dimLength,originVector)-mesh.C())).value(); 
    ...
    while (runTime.loop())
    {
		...
		for (label cellI=0; cellI<mesh.C().size(); cellI++)
		{
			...
			myp[cellI] = calculatePressure(runTime.time().value(), mesh.C()[cellI], originVector, rFarCell);
		...
		myU = fvc::grad(myp)*dimensionedScalar("tmp",dimTime,1.);
